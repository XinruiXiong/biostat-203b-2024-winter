...
### To read some data that not in mimic_icu_cohert from the BigQuery database:
  ...
  output$plot1 <- renderPlot({
    if (input$VOI == "Race") {
      ...
      cohort_long1 <- mimic_icu_cohort %>%
        pivot_longer(cols = c("White Blood Cells","Creatinine","Potassium","Glucose","Sodium","Chloride","Bicarbonate","Hematocrit"), names_to = "Variable", values_to = "Value")
      if(input$RMO==TRUE){
        ggplot(cohort_long1,aes(x = Variable, y = Value)) +
          geom_boxplot(outlier.shape = NA) +
          scale_y_continuous(limits = c(0, 
                                        max(quantile(cohort_long1$Value, 0.75,na.rm=TRUE) + 1.5 * IQR(cohort_long1$Value,na.rm=TRUE))),)+
      ...
      cohort_long2 <- mimic_icu_cohort %>%
        pivot_longer(cols = c("Heart Rate","Non Invasive Blood Pressure systolic","Temperature Fahrenheit","Respiratory Rate", "Non Invasive Blood Pressure diastolic"), names_to = "Variable", values_to = "Value")
      if(input$RMO==TRUE){
        ggplot(cohort_long2,aes(x = Variable, y = Value)) +
          geom_boxplot(outlier.shape = NA) +
          scale_y_continuous(limits = c(0, 
                                        max(quantile(cohort_long2$Value, 0.75,na.rm=TRUE) + 1.5 * IQR(cohort_long2$Value,na.rm=TRUE))),)+
          labs(title = "Boxplot of Lab Measurements", 
               x = "Variable", 
               y = "Value")+
          coord_flip()
      ...
      ggplot(mimic_icu_cohort, aes(x=last_careunit)) + 
        geom_bar(stat="count",fill="blue") +
        labs(title = "Last Care Unit distribution", x = "last_careunit", y = "count")+
        coord_flip()
    } 
  })
  ...
  output$plot2 <- renderPlot({
    ...
    ggplot() +
      ...
      labs(
        x = "", 
        y = "",
        title = str_c("Patient ", sid, ", ", pull(sid_pat,gender), ", ",pull(sid_pat,anchor_age), " years old ",unique(pull(sid_adm,race))),
