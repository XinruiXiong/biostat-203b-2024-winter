...
![](images/Screenshot%202024-01-26%20at%202.05.15%20PM.png)
...
**'zcat' somehow doesn't work in my mac machine, so I use 'gzcat' instead in the rest of the qmd file.**
...
echo "count the lines admissions.csv.gz (without headlines)"
...
echo "count the patients numbers in paitients.csv.gz (without headline)"
...
```{bash, eval=TRUE}
echo "list the unique values (without headline)"
echo "admission_type:"
...
echo -e "\ncount the unique values (without headline)"
echo "admission_type:"
gzcat ~/mimic/hosp/admissions.csv.gz |tail -n +2| cut -d ',' -f 6 | sort | uniq -c
echo "admission_location:"
gzcat ~/mimic/hosp/admissions.csv.gz |tail -n +2| cut -d ',' -f 8 | sort | uniq -c
echo "insurance:"
gzcat ~/mimic/hosp/admissions.csv.gz |tail -n +2| cut -d ',' -f 10 | sort | uniq -c
echo "race:"
gzcat ~/mimic/hosp/admissions.csv.gz |tail -n +2| cut -d ',' -f 13 | sort | uniq -c
...
do
  echo $char
  # only grep the whole independent word
  grep -o -w $char ~/pg42671.txt | wc -l
done
```

The `wget -nc` will download files from Internet through given URL without overwriting exists files.
...
![](images/Screenshot%202024-01-26%20at%202.01.22%20PM.png)
