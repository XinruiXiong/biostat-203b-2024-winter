...
mimic_pred <- mimic_pred %>%
  mutate(.pred_class = if_else(.pred_FALSE > 0.5, "FALSE", "TRUE"))%>%
  mutate(.pred_class = factor(.pred_class, levels = c("FALSE", "TRUE")))
mimic_pred$los_long <- factor(mimic_pred$los_long, levels = c("FALSE", "TRUE"))
